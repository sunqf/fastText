...
namespace fasttext {
  ...
  ALSModel::ALSModel(std::shared_ptr<Matrix> first_embedding, std::shared_ptr<Matrix> first_w1,
  ...
  void ALSModel::computeHidden(const std::shared_ptr<Matrix> embedding,
  ...
  real ALSModel::predict(const std::vector<int32_t>& first,
...
*/
  ...
  void ALSModel::update(const std::vector<int32_t>& input,
  ...
  void ALSModel::update(const std::vector<int32_t>& first_input,
    ...
    real prob = dot(first_output_, second_output_);
     ...
     */
    loss_ += powf(real(label) - prob, 2.0);
    nexamples_ += 1;

    real alpha = weight * lr * 2 * (real(label) - prob);
  ...
  void ALSModel::getFirstOutput(const std::vector<int32_t>& words, Vector& output) const {
    Vector hidden(args_->dim);
    computeHidden(first_embedding_, words, hidden);
    output.mul(*first_w1_, hidden);
  }

  void ALSModel::getSecondOutput(const std::vector<int32_t>& words, Vector& output) const {
    Vector hidden(args_->dim);
    computeHidden(second_embedding_, words, hidden);
    output.mul(*first_w1_, hidden);
  }

  real ALSModel::similarity(Vector &first, Vector &second) const {
  ...
  real ALSModel::firstSimilarity(const std::vector<int32_t> &first_words,
  ...
  real ALSModel::secondSimilarity(const std::vector<int32_t> &first_words,
                                   const std::vector<int32_t> &second_words) const {
    Vector first_output(args_->dim), second_output(args_->dim);
...
*/
  ...
  real ALSModel::log(real x) const {
    if (x > 1.0) {
      return 0.;
    }
  ...
  real ALSModel::sigmoid(real x) const {
    // x != x  check x is NaN
    if (x != x) {
      return 0.0;
